    ...
    const void *buf, dmu_tx_t *tx, uint32_t flags)
{
	...
	if (flags & DMU_DIRECTIO) {
		...
		if (error == EINVAL) {
			...
			error = dmu_write_abd(dn, offset, size, data,
			    DMU_DIRECTIO, tx);
			abd_free(data);
			return (error);
		}
    ...
    dmu_tx_t *tx, uint32_t flags)
{
	...
	if (offset == db->db.db_offset && blksz == db->db.db_size) {
		if (flags & DMU_DIRECTIO) {
			int error = dmu_check_direct_valid(dn, offset,
			    blksz, B_TRUE);

			if (error == EINVAL) {
				return (error);
			} else if (error == 0) {
				arc_buf_flags_t orig_flags = buf->b_flags;
				abd_t *data = abd_get_from_buf(buf->b_data,
				    blksz);

				buf->b_flags |= ARC_BUF_FLAG_LOANED_DIRECT;

				/*
				 * Note that dmu_direct_done() will call
				 * arc_return_buf() and arc_buf_destroy().
				 */
				error = dmu_write_direct(db, data, tx);
				if (error) {
					buf->b_flags = orig_flags;
					abd_free(data);
					return (error);
				}
