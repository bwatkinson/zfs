    ...
    arc_buf_t *buf, void *vdb)
{
	...
	/*
	 * In the event that we were attempting to cache a Direct IO
	 * dbuf in the ARC, we never released the db_mtx in dbuf_read_impl.
	 * We will transfer ownership of the db_mtx here.
	 */
	if (db->db_transferring_ownership == TRUE) {
		mutex_transfer_ownership(&db->db_mtx);
		db->db_transferring_ownership = FALSE;
	} else {
		mutex_enter(&db->db_mtx);
	}
	ASSERT(MUTEX_HELD(&db->db_mtx));
    ...
    db_lock_type_t dblt, void *tag, blkptr_t *bp)
{
	...
	/*
	 * We are reading from a Direct IO dbuf, so we must hold the
	 * mutex across the arc_read() and wait for arc_read() to
	 * complete. This ensures that any other Direct IO write's
	 * do not interfere with the current read.
	 *
	 * The locking here is a bit complex. We can not drop the
	 * db_mtx and just have another reader in dbuf_read()
	 * start working because another Direct IO write may come in
	 * and transistion the dbuf to another state. The only way to
	 * make sure that the dbuf stays consistent is to hold the db_mtx
	 * and get the dbuf back into a stable state with data.
	 */
	boolean_t wait_arc_read = B_FALSE;
	if (db->db_blkptr != bp) {
		wait_arc_read = B_TRUE;
		aflags = ARC_FLAG_WAIT;
		db->db_transferring_ownership = TRUE;
	} else {
		aflags = ARC_FLAG_NOWAIT;
	}
...
dbuf_read(dmu_buf_impl_t *db, zio_t *zio, uint32_t flags)
{
	...
	/*
	 * We are simply using the db_rwlock as a synchronization in the
	 * case of Direct IO. If a Direct IO write is currently in progress
	 * we will wait and then grab the db_mtx. After that we quickly drop
	 * the db_rwlock as we were merely using it for synchronization
	 * purposes.
	 */
...
dbuf_undirty(dmu_buf_impl_t *db, dmu_tx_t *tx)
{
	...
	if (db->db_state != DB_NOFILL) {
		 ...
		 * maybe UNCACHED, so we to not need to destroy an ARC buffer.
		 */
