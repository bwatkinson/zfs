...
zfs_read(struct znode *zp, zfs_uio_t *uio, int ioflag, cred_t *cr)
{
	...
	/*
	 * O_DIRECT requests may be split between buffered and direct IO
	 * due to the recordsize alignment restriction.  Account for both
	 * the direct and any remaining buffered portion.
	 */
	dataset_kstats_t *ks = &zfsvfs->z_kstat;
	if (uio->uio_extflg & UIO_DIRECT) {
		int64_t ndirect = zfs_uio_dio_nbytes(uio);

		ASSERT3S(nread, >=, ndirect);
		nread -= ndirect;

		if (ndirect > 0)
			dataset_kstats_update_direct_read_kstats(ks, ndirect);

		if (nread > 0)
			dataset_kstats_update_read_kstats(ks, nread);
	} else {
		dataset_kstats_update_read_kstats(ks, nread);
	}
out:
...
zfs_write(znode_t *zp, zfs_uio_t *uio, int ioflag, cred_t *cr)
{
	...
	if (uio->uio_extflg & UIO_DIRECT) {
		...
		ASSERT3S(nwritten, >=, ndirect);
		nwritten -= ndirect;

		if (ndirect > 0)
			dataset_kstats_update_direct_write_kstats(ks, ndirect);

		if (nwritten > 0)
			dataset_kstats_update_write_kstats(ks, nwritten);
	} else {
		dataset_kstats_update_write_kstats(ks, nwritten);
	}
