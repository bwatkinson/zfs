    ...
    enum zio_stage pipeline)
{
    ...
    zio->is_zio_struct = ZIO_STRUCT_FLAG;
    zio->zio_pid = getpid();
    zio->zlog_type = zio_ignore;
	zio->io_spa = spa;
	zio->io_txg = txg;
	zio->io_done = done;
...
#ifdef _KERNEL
        cb_zio_pipeline_args_t cb_args = create_cb_zio_pipeline_args(zio, stage);
...
zio_wait(zio_t *zio)
{
	...
	while (zio->io_executor != NULL) {
        if (zio->zlog_type == zio_is_read) {
		    error = cv_timedwait_io(&zio->io_cv, &zio->io_lock,
		        ddi_get_lbolt() + timeout, zio);
        } else {
		    error = cv_timedwait_io(&zio->io_cv, &zio->io_lock,
		        ddi_get_lbolt() + timeout, zio);
        }
