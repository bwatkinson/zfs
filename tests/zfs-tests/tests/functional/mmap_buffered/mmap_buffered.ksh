...
. $STF_SUITE/tests/functional/mmap_buffered/mmap.cfg

#
# DESCRIPTION:
# 	Verify mixed buffered IO and mmap IO.
#
# STRATEGY:
#	1. Create an empty file.
#	2. Start a background fio randomly buffered reading/writing to the file.
#	3. Start a background fio randomly mmap reading/writing to the file.
#
...
log_assert "Verify mixed buffered IO and mmap IO"
...
log_must eval "fio --filename=$tmp_file --name=buffer-write \
	--rw=write --size=$size --bs=$bs --direct=0 --numjobs=1 \
...
log_pass "Verfied mixed bufferedt IO and mmap IO"
