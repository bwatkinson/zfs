...
AC_DEFUN([ZFS_AC_KERNEL_GET_USER_PAGES], [
	...
	ZFS_LINUX_TEST_RESULT([get_user_pages_unlocked_gup_flags], [
		...
		dnl #
		AC_MSG_CHECKING([whether get_user_pages_unlocked() takes ]
		    [write flag])
		ZFS_LINUX_TEST_RESULT([get_user_pages_unlocked_write_flag], [
			...
			dnl #
			AC_MSG_CHECKING([whether get_user_pages_unlocked() ]
			    [takes struct task_struct])
			ZFS_LINUX_TEST_RESULT(
			    [get_user_pages_unlocked_task_struct], [
				AC_MSG_RESULT(yes)
				AC_DEFINE(
				    HAVE_GET_USER_PAGES_UNLOCKED_TASK_STRUCT, 1,
				    [get_user_pages_unlocked() takes struct ]
				    [task_struct])
			], [
				AC_MSG_RESULT(no)
				...
				dnl # get_user_pages
				dnl #
				AC_MSG_CHECKING([whether get_user_pages() ]
				    [takes struct task_struct])
				ZFS_LINUX_TEST_RESULT(
					...
					AC_DEFINE(
					    ...
					    [get_user_pages() takes struct ]
					    [task_struct])
				], [
					dnl #
					dnl # If we cannot map the user's
					dnl # pages in then we cannot do Direct IO
					dnl #
