dnl #
dnl # ZERO_PAGE() is an alias for empty_zero_page. On certain architectures
...
AC_DEFUN([ZFS_AC_KERNEL_ZERO_PAGE], [
	AC_MSG_CHECKING([Whether ZERO_PAGE() is available])
	ZFS_LINUX_TEST_RESULT([zero_page], [
		AC_MSG_RESULT(yes)
		AC_DEFINE(HAVE_ZERO_PAGE, 1,
		    [ZERO_PAGE() is available])
